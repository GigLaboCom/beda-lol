...
3. Run the script in `build` before `astro build` via Nx (`og` target with inputs = script + core + fonts, outputs = `public/og/**`) so it is cached and not re-rendered when nothing changed. Report build time.
