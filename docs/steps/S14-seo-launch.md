...
## Goal
The stage 1 site is live on `beda.lol` with correct robots, sitemap, canonical URLs, structured data and performance, registered in Google Search Console and Yandex Webmaster.
...
2. `public/robots.txt`: disallow `/api/`, `/admin/`, `/kabinet`, `/moya-yahta`, `/dev/`; `Sitemap: https://beda.lol/sitemap-index.xml`.
...
## Verify
- `curl -I https://beda.lol/` → 200, HSTS, CSP present; `/robots.txt`, `/sitemap-index.xml` valid.
